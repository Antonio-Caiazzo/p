	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("action") != null && request.getParameter("action").compareTo("dettaglio") == 0) {
			...
			try {
				...
				response.sendRedirect("../webapp/errorpage.html");
			} finally {
			...
			dispatcher.forward(request, response);
				...
				&& request.getParameter("action").compareTo("modificaForm") == 0) {
			ProductBean bean;
			try {
				...
				&& request.getParameter("action").compareTo("modifica") == 0) {
			...
			if (request.getSession().getAttribute("carrello") != null) {
				...
				CartBean newCart = cartmodel.updateCarrello(bean,
						(CartBean) request.getSession().getAttribute("carrello"));
			...
			if (request.getSession().getAttribute("preferiti") != null) {
				...
				@SuppressWarnings("unchecked")
				Collection<ProductBean> lista = preferitiModel.updatePreferiti(bean,
						(Collection<ProductBean>) request.getSession().getAttribute("preferiti"));
			...
			RequestDispatcher dispatcher = getServletContext()
					.getRequestDispatcher("/ProductsPage.jsp?tipologia=" + tipologia);
			dispatcher.forward(request, response);
		}
	}
